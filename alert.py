...
def get_alert_info():
    metrics_list = {
        'users_feed': {
            ...
            'metric': 'Users Feed',
            'table_name': 'simulator_20211220.feed_actions'
        },
        'likes': {
            'alias': 'likes',
        ...
        'lpu': {
            ...
            'metric': 'Likes per user',
            'table_name': 'simulator_20211220.feed_actions'
        },
        'vpu': {
            'alias': 'lpu',
            'formula': "countIf(user_id, action='view')/uniqExact(user_id)",
            'metric': 'Views per user',
            'table_name': 'simulator_20211220.feed_actions'
        },
        'users_msg': {
            'alias': 'users_msg',
            'formula': "uniqExact(user_id)",
            'metric': 'Users messenger',
            'table_name': 'simulator_20211220.message_actions'
        },
        'messages': {
            'alias': 'messages',
    ...
    '''
    responsible_users = {
        'metrics': {'users_msg': '@marsellios'},
        'groups': {'os': '@marsellios'}
    }
    alert_plots = {
        'total': {
    ...
    return metrics_list, group_by_slices_list, where_expression_template, responsible_users, alert_plots


def get_select_vars(metrics, metrics_list, group_by_slices_list, slice, all_data=False):
    date_select = '''
        toDate(time) as date,
    '''
...
def get_from_table_name(metrics, metrics_list):
    table_name_list = []
    for metric in metrics:
...
def get_where_expression(where_expression_template, group_by_slices_list, time, slice, all_data=True):
    ...
    day_1_max_time = time + pd.Timedelta(minutes=15) - pd.Timedelta(days=1)
    day_7_min_time = time - pd.Timedelta(minutes=15) - pd.Timedelta(days=7)
    day_7_max_time = time + pd.Timedelta(minutes=15) - pd.Timedelta(days=7)
    ...
    '''.format(day_0_min_time=day_0_min_time,
...
def send_alert(plot_df, group, metric_name, metric_alias, slice_name, alert_plot, responsible_users, chat):
    bot = telegram.Bot(os.environ.get('REPORT_BOT_TOKEN'))

    dashboard_link = 'https://superset.lab.karpov.courses/superset/dashboard/201/'
    ...
    if slice_name == 'total':
        ...
        График алерта {}\n Дашборд {}""".format(metric_name, alert_plot, dashboard_link)
    else:
        text = """Проблема с метрикой {} - сильное отклонение от вчера/неделю назад\nВ срезе {} - {}\n
        График алерта {}\n Дашборд {}""".format(metric_name, slice_name, group, alert_plot, dashboard_link)
    ...
    bot.sendMessage(chat_id=chat, text=text, parse_mode='HTML')
...
def get_alert(alert_data, time, chat):
    chat_id = chat
    metrics_list, group_by_slices_list, where_expression_template, responsible_users, alert_plots = get_alert_info()
    metrics = alert_data['metrics']
    slice = alert_data['slice']

    query = '''SELECT {select_vars} FROM {from_table_name} WHERE {where_expression} GROUP BY {group_by_expression}
    '''

    select_vars = get_select_vars(metrics, metrics_list, group_by_slices_list, slice)
    from_table_name = get_from_table_name(metrics, metrics_list)
    ...
    alert_query = query.format(select_vars=select_vars,
                               ...
                               group_by_expression=group_by_expression)  # Body of the query
    print(alert_query)

    all_data = Getch(alert_query).df  # save our query to data frame
    print(all_data)
    data_to_alerts = {}
    ...
    for group in data_to_alerts:
        ...
        for metric in metrics:
            ...
            metric_name = metrics_list[metric]['metric']
            alert_data = get_metric_alert(alert_df, metric, metric_alias, slice, group, day_threshhold=0.3)
            ...
            alert_log_df = pd.DataFrame(alert_data, index=[0]).fillna(0)
            alert_log_df = alert_log_df[['time', 'day_0_value', 'day_1_value', 'day_7_value',
                                         'day_1_diff', 'day_7_diff', 'slice', 'group_level',
                                         'metric', 'metric_name', 'is_alert']]
            # !!!
            Insertch(alert_log_df, 'alerts_log').insert

            if not alert_data['is_alert'] == 1:
                continue

            select_vars = get_select_vars(metrics, metrics_list, group_by_slices_list, slice, True)
            from_table_name = get_from_table_name(metrics, metrics_list)
            where_expression = get_where_expression(where_expression_template, group_by_slices_list, time, slice, True)
            group_by_expression = get_group_by_expression(group_by_slices_list, slice, True)
            ...
            send_alert(plot_df, group, metric_name, metric_alias, slice_name, alert_plot, responsible_users,
                       chat=chat_id)
    return
...
def get_metric_alert(alert_df, metric, metric_alias, slice, group, day_threshhold=0.3):
    ...
    if last_time != '1970-01-01 03:00:00':
        return alert_data


    if (day_1_value <= day_0_value <= day_7_value) or (day_7_value <= day_0_value <= day_1_value):
        return alert_data
    if day_1_diff >= day_threshhold and day_7_diff >= day_threshhold:
        is_alert = 1
        alert_data['is_alert'] = is_alert
        return alert_data
    else:
...
def global_monitoring(chat=None):
    ...
    alerts = [
        {'metrics': ['users_feed', 'likes', 'views', 'ctr'], 'slice': 'total'},
        {'metrics': ['users_feed', 'likes', 'views', 'ctr'], 'slice': 'os'},
        {'metrics': ['users_msg', 'messages', 'mpu'], 'slice': 'total'},
        ]

    time = Getch(''' 
    SELECT toStartOfFifteenMinutes(max(time)) - interval 15 minute as max_time
    FROM simulator_20211220.feed_actions
    WHERE toDate(time) >= today() - 1
     ''').df
    time = time['max_time'].iloc[0]  # pull out the maximum full 15 minutes of today
